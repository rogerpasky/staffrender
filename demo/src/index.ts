...
let visualizerRight: ssv.StaffSVGVisualizer;
let visualizerLeft: ssv.StaffSVGVisualizer;
...
function initPlayerAndVisualizer1(seq: mm.INoteSequence) {
  ...
  visualizerRight = new ssv.StaffSVGVisualizer(seq, staffRight, configRight);
  visualizerLeft = new ssv.StaffSVGVisualizer(seq, staffLeft, configLeft);
...
let visualizer2: ssv.StaffSVGVisualizer;
...
let configSignatures: ssv.StaffSVGVisualizerConfig = {
  noteHeight: 15,
  pixelsPerTimeStep: 0,
  scrollType: ssv.ScrollType.NOTE
};
...
function initPlayerAndVisualizer2(seq: mm.INoteSequence) {
  ...
  visualizer2 = new ssv.StaffSVGVisualizer(seq, signatures, configSignatures);
...
let configList: ssv.StaffSVGVisualizerConfig[] = [];
    ...
    (seq) => {
      ...
      seq.notes.forEach(
        (note: mm.NoteSequence.Note) => {
          if (instruments.indexOf(note.instrument) < 0) {
      ...
      instruments.forEach(
        instrument => {
          let configMidi: ssv.StaffSVGVisualizerConfig = {
            noteHeight: 8,
            pixelsPerTimeStep: UNIFORM_TIME_SIZE,
            instruments: [instrument],
            scrollType: ssv.ScrollType.PAGE
          };
...
function initPlayerAndVisualizer3(seq: mm.INoteSequence) {
  ...
  configList.forEach(
    config => {
      ...
      visualizers.push(new ssv.StaffSVGVisualizer(seq, visualizerDiv, config));
    }
  );
  ...
  const config: ssv.VisualizerConfig = {
    noteHeight: 8,
    pixelsPerTimeStep: UNIFORM_TIME_SIZE,
  };
...
let visualizer4: ssv.StaffSVGVisualizer;
...
let configCatalog: ssv.StaffSVGVisualizerConfig = {
  noteHeight: 15,
  pixelsPerTimeStep: 0,
  scrollType: ssv.ScrollType.NOTE
};
...
function initPlayerAndVisualizer4(seq: mm.INoteSequence) {
  ...
  visualizer4 = new ssv.StaffSVGVisualizer(seq, catalog, configCatalog);
...
function appendQuantized(
  ...
  // BEWARE!!! NoteSequence Protobuf definition misses a quantized signature 
  // handling. It must be set in 'time' instead of 'quantizedStartStep' doing
  // the proper conversion according to first qpm as a workaround.
  const timeOffset = 
    offset / sequence.quantizationInfo.stepsPerQuarter * 60 / sequence.tempos[0].qpm;
  appended.keySignatures.forEach(
    (ks: mm.NoteSequence.IKeySignature) => { 
      sequence.keySignatures.push(
        {
          time: ks.time + timeOffset, 
          key:ks.key
        }
      ); 
    }
  ...
  appended.timeSignatures.forEach(
    (ts: mm.NoteSequence.ITempo) => {
      sequence.timeSignatures.push(
        {
          time: ts.time + timeOffset, 
